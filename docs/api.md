...
| 方法 | 路径 | 说明 |
|---|---|---|
| `GET` | `/forensics/keys` | 公开：全部签名公钥（含已轮换 retired 密钥） |
| `GET` | `/forensics/credentials/:id` | 公开：按凭证 ID 校验签名，返回白名单声明 |
| `GET` | `/forensics/media/:id/credentials` | 所有者/审核员：某媒体的全部凭证 |

公开校验响应只包含签发时白名单内的声明（处理图 SHA-256、尺寸、时间戳、水印指纹、删除范围），不包含隔离桶键、原图哈希、文件名或所有者信息；公开地址无法用于定位或还原原图。凭证 `payload` 为签名原文，任何方可结合 `/forensics/keys` 中的公钥离线复核 Ed25519 签名。
